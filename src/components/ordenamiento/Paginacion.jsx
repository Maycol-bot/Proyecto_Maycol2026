...
const Paginacion =({
    ...
    establecerRegistroPorPagina
}) => {
        const totalPaginas = Math.ceil(totalRegistros / registroPorPagina);
        ...
        const cambiarCantidadRegistros = (e) => {
            establecerRegistroPorPagina(Number(evento.target.value));
            establecerPaginaActual(1); // Reiniciar a la primera página al cambiar la cantidad de registros
        };

        const elementosPaginacion = [];
        const maximoPaginasAMostrar = 3; // Número máximo de páginas a mostrar en la paginación
        ...
        if (paginaFin - paginaInicio + 1 < maximoPaginasAMostrar) {
            paginaInicio = Math.max(
                1, paginaFin - maximoPaginasAMostrar + 1
            );
        }
